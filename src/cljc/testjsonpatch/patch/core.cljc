(ns testjsonpatch.patch.core
  (:use [testjsonpatch.patch.util :only [apply-patch diff* *keywordize*]]))
...
(defn call-diff*
  [obj1 obj2 prefix]
  #?(:cljs (do (log "Obj1 in call-diff: " (str (js->clj (.parse js/JSON obj1))))
               (log "Obj2 in call-diff: " (str (js->clj (.parse js/JSON obj2))))))
